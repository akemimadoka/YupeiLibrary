...
#include "..\Comparators.h" //for less<> etc.
#include <cstddef>
...
namespace Yupei
{
	...
	{
		template<typename D>
		using DeleterHasPointerOp = typename std::remove_reference_t<D>::pointer;
	...
	public:
		...
		using RefType = std::conditional_t<std::is_reference<deleter_type>::value, deleter_type, const deleter_type&>;
